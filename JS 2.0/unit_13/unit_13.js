...
function f6() {
    let inputKey = document.querySelector('.i-61');
    let inputValue = document.querySelector('.i-62');

    a6[inputKey.value] = inputValue.value;
...
function f7() {
    ...
    for (let key in a7) {
        if (a7.hasOwnProperty(input.value)) {
            out.textContent = 1;
        } else {
            out.textContent = 0;
        }
...
function f8() {
    ...
    for (let key in a8) {
        if (a8.hasOwnProperty(input.value)) {
            out.textContent = a8[key];
        } else {
            out.textContent = 0;
        }
    }
...
function f19() {
    ...
    for (let key in a19) {
        for (let i = 0; i < a19[key].length; i++) {
            if (input.value.toLowerCase() === a19[key][i].toLowerCase()) {
                result = key + '';
            } else {
                result += '';
            }
