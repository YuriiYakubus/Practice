...
let input_4 = document.querySelector('.i-4');

function f4() {
    if (s4.has(input_4.value)) {
        out_4.textContent = true;
    } else {
        out_4.textContent = false;
    }
}
...
function f6() {
    let sorted = new Set(a6);

    out_6.textContent = sorted.size;
}
...
function f7() {
    let pass = input_7.value;
    let set = new Set(input_7.value);

    if (set.size > 6 && set.size === pass.length) {
        out_7.textContent = 1;
    } else {
        out_7.textContent = 0;
    }
...
function f10(out_set, elem) {
    ...
    document.querySelector(elem).textContent = res;
}
...
function f13() {
    let lowerCasedString = str13.toLowerCase();
    let arr = lowerCasedString.split('');
    let set = new Set(arr);
    let counter = 0;
    let obj = {};
